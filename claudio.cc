...
void claudio::close_link () {
  ...
  if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_CloseDigitizer(" << handle_ << "): " << caen_error (err);
}


void claudio::init_link () {
  CAEN_DGTZ_ErrorCode err = CAEN_DGTZ_OpenDigitizer (CAEN_DGTZ_PCI_OpticalLink, 0, 0, 0, &handle_);
  if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_OpenDigitizer(" << CAEN_DGTZ_PCI_OpticalLink << "," << 4 << "," << 0 << "," << 0 << "): " << caen_error (err);

  err = CAEN_DGTZ_Reset (handle_);
  if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_Reset(" << handle_ << "):" << caen_error (err);
  usleep (100000);
}

void claudio::init_channels () {
  CAEN_DGTZ_ErrorCode err = CAEN_DGTZ_SetDESMode (handle_, CAEN_DGTZ_DISABLE);
  if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_SetDESMode(" << handle_ << ",true): " << caen_error (err);

  int dc_offset = 0x2700;
  err = CAEN_DGTZ_SetChannelDCOffset (handle_, 1, dc_offset);
  if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_SetChannelDCOffset(" << handle_ << "," << 0 << "," << dc_offset << "): " << caen_error (err);

  u_int8_t channels_mask = 2;
  err = CAEN_DGTZ_SetChannelEnableMask (handle_, channels_mask);
  if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_SetChannelEnableMask(" << handle_ << "," << channels_mask << "): " << caen_error (err);
}
...
void claudio::init_trigger () {
  ...
  if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_SetExtTriggerInputMode(" << handle_ << ",CAEN_DGTZ_TRGMODE_ACQ_AND_EXTOUT): " << caen_error (err);

  err = CAEN_DGTZ_SetSWTriggerMode(handle_, CAEN_DGTZ_TRGMODE_ACQ_AND_EXTOUT);
  if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_SetSWTriggerMode(" << handle_ << ",CAEN_DGTZ_TRGMODE_ACQ_AND_EXTOUT): " << caen_error (err);

  err = CAEN_DGTZ_SetChannelSelfTrigger (handle_, CAEN_DGTZ_TRGMODE_DISABLED, 0x2);
  if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_SetChannelSelfTrigger(" << handle_ << ",CAEN_DGTZ_TRGMODE_DISABLED,0xff): " << caen_error (err);

  err = CAEN_DGTZ_SetIOLevel (handle_, CAEN_DGTZ_IOLevel_TTL);
  if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_SetIOLevel(" << handle_ << "," << CAEN_DGTZ_IOLevel_TTL << "): " << caen_error (err);

  u_int32_t record_length = 10000;
  err = CAEN_DGTZ_SetRecordLength (handle_, record_length);
  if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_SetRecordLength(" << handle_ << "," << record_length << "): " << caen_error (err);

  int post_trigger = 50;
  err = CAEN_DGTZ_SetPostTriggerSize (handle_, post_trigger);
  if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_SetPostTriggerSize(" << handle_ << "," << post_trigger << "): " << caen_error (err);

  err = CAEN_DGTZ_SetAcquisitionMode (handle_, CAEN_DGTZ_SW_CONTROLLED);
  if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_SetAcquisitionMode(" << handle_ << "," << CAEN_DGTZ_SW_CONTROLLED << "): " << caen_error (err);
}
...
void claudio::init_buffers () {
  ...
  if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_SetMaxNumEventsBLT(" << handle_ << ","<< max_events_blt << "): " << caen_error (err);

  err = CAEN_DGTZ_SetInterruptConfig (handle_, CAEN_DGTZ_ENABLE, 1, 1, 1, CAEN_DGTZ_IRQ_MODE_RORA);
  if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_SetInterruptConfig(" << handle_ << ",CAEN_DGTZ_ENABLE,1,1,CAEN_DGTZ_IRQ_MODE_RORA): " << caen_error (err);

  err = CAEN_DGTZ_MallocReadoutBuffer (handle_, &event_buffer_, &buffer_size_);
  if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_MallocReadoutBuffer(" << handle_ << "): " << caen_error (err);
}

void claudio::start () {
  CAEN_DGTZ_ErrorCode err = CAEN_DGTZ_SWStartAcquisition (handle_);
  if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_SWStartAcquisition(" << handle_ << "): " << caen_error (err);
  usleep (10000);
}

void claudio::stop () {
  CAEN_DGTZ_ErrorCode err = CAEN_DGTZ_SWStopAcquisition (handle_);
  if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_SWStopAcquisition(" << handle_ << "): " << caen_error (err);
}
...
std::vector<claudio_ev*> claudio::loop() {
  ...
  if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_SendSWtrigger(" << handle_ << "): " << caen_error (err);
  ...
  for (int j = 0; j < 5; j++) {
    ...
    if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_ReadData(" << handle_ << ",CAEN_DGTZ_SLAVE_TERMINATED_READOUT_MBLT," << event_buffer_ << "," << gross_size << "): " << caen_error (err);

    uint32_t n_events;
    err = CAEN_DGTZ_GetNumEvents (handle_, event_buffer_, gross_size, &n_events);
    if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_GetNumEvents(" << handle_ << "," << event_buffer_ << "," << gross_size << "," << n_events << "): " << caen_error (err);
    ...
    for (uint32_t i = 0; i < n_events; i++) {
      ...
      if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_GetEventInfo(" << handle_ << "," << event_buffer_ << "," << gross_size << "," << i << "," << event_info.EventCounter << "," << event_ptr << "): " << caen_error (err);
...
void claudio::set_register (uint16_t reg, uint32_t val) {
  ...
  if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_WriteRegister(" << handle_ << "," << std::hex << std::showbase << reg << "," << val << "): " << caen_error (err);
}
...
uint32_t claudio::get_register (uint16_t reg) {
  ...
  if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_ReadRegister(" << handle_ << "," << std::hex << std::showbase << reg << "): " << caen_error (err);
...
bool claudio::wait_irq () {
  ...
  else if (err != CAEN_DGTZ_Success) std::cout << " CAEN_DGTZ_IRQWait(" << handle_ << "," << 100 << "): " << caen_error (err);
