...
sibilla::sibilla (): desc_() {
  ...
  trg.add_options()
    ...
    ("events,e", po::value<int>()->default_value(-1), "events to acquire (-1 for infinite)")
    ("duration,Q", po::value<int>()->default_value(-1), "time to acquire in seconds (-1 for infinite)")
  ...
  ch.add_options()
    ...
    ("dc-offset,D", po::value<std::vector<int>>()->default_value(std::vector<int>{0xefff}, "{0xefff}"), "channel dc offset (0x1000 for positive pulses and 0xefff for negative pulses)") 
    ("channel-threshold,T", po::value<std::vector<int>>()->default_value(std::vector<int>{-1}, "{-1}"), "channel threshold for auto-trigger, negative means disabled (value in samples)")
